...
plt.title("Correlation Matrix (Salary Dataset)")
plt.show()

# 3. Build predictive model
X = df_salary.iloc[:, :-1].values        # YearsExperience
y = df_salary.iloc[:, -1].values         # Salary
...
fig_data, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
...
# 4. Analyze normality of errors (residuals)
...
fig_residual, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
...
df_startups = pd.read_csv("Data/50_Startups.csv")
print(f"\nNumber of rows: {df_startups.shape[0]}")
print(f"Number of columns: {df_startups.shape[1]}")
print("\nFirst few rows:")
print(df_startups.head(10))

print("\nBasic statistics:")
print(df_startups.describe())

# Correlation matrix (numeric columns only)
correlation = df_startups.corr(numeric_only=True)
print("\nCorrelation matrix:")
print(correlation)

plt.figure(figsize=(8, 6))
sns.heatmap(correlation, annot=True, cmap='coolwarm', center=0)
plt.title("Correlation Matrix (Startups Dataset)")
plt.show()

# Features and target
X = df_startups.iloc[:, :-1].values      # all predictors
y = df_startups.iloc[:, -1].values       # Profit

# Visualize each numeric feature vs Profit
for col in ["R&D Spend", "Administration", "Marketing Spend"]:
    plt.figure(figsize=(6, 4))
    plt.scatter(df_startups[col], df_startups["Profit"], alpha=0.7)
...
# ============================================
# PART 3: POLYNOMIAL REGRESSION – Position Salaries
# ============================================
...
df_position = pd.read_csv("Data/Position_Salaries.csv")

print("\nFirst few rows:")
print(df_position.head())

X = df_position.iloc[:, 1:2].values   # Level
...
fig_polynomial, axes = plt.subplots(2, 3, figsize=(16, 10))
axes = axes.ravel()
...
for i, degree in enumerate(degrees):
    ...
    axes[i].set_title(f"Polynomial Deg {degree}\nR²={r2:.4f}")
...
# ============================================
# PART 3B: POLYNOMIAL REGRESSION – Simulated Data
# ============================================
...
# X-Transformation to do linear regression
X_transformed = (1 / (loading_time ** 2)).reshape(-1, 1)
